...
public class BoardController {
	...
	@RequestMapping(value = "/{articleNO}", method = RequestMethod.GET) // GET방식으로 요청하면서 글 번호를 전달하므로 글 번호에 대한 글 정보를 조회한다.
	...
	//수정하기
	@RequestMapping(value = "/{articleNO}", method = RequestMethod.PUT)  // PUT방식으로 요청하므로 articleNO에 대한 글을 전달되는 JSON 정보로 수정한다.
	public ResponseEntity<String> modArticle (@PathVariable("articleNO") Integer articleNO, @RequestBody ArticleVO articleVO) {
		// 전송된 JSON 회원 정보를 바로 ArticleVO 객체의 속성에 설정한다.
	...
	//삭제하기
	@RequestMapping(value = "/{articleNO}", method = RequestMethod.DELETE) // DELETE 방식으로 요청하므로 전달되는 articleNO에 대한 글을 삭제한다.
