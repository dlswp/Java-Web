...
public class BoardController extends HttpServlet {
	...
	private void doHandle(HttpServletRequest request, HttpServletResponse response)
		...
		HttpSession session;
		// 답글에 대한 부모 글 번호를 저장하기 위해 세션을 사용한다.
...
//				nextPage = "/board06/listArticles.jsp";
				...
				// 최초 요청 시 section 값과 pageNum 값이 없으면 각각 1로 초기화한다.
				
				Map pagingMap = new HashMap();
				pagingMap.put("section", section);
				pagingMap.put("pageNum", pageNum);
				// section 값과 pageNum 값을 HashMap에 저장한 후 메서드로 넘김니다.
				
				Map articlesMap = boardService.listArticles(pagingMap);
				// section 값과 pageNum 값으로 해당 섹션과 페이지에 해당되는 글 목록을 조회한다.
				
				articlesMap.put("section", section);
				articlesMap.put("pageNum", pageNum);
				// 브라우저에서 전송된 section과 pageNum 값을 articlesMap에 저장한 후 listArticles.jsp에 넘긴다.
...
//				nextPage = "/board06/listArticles.jsp";
				...
				int section = Integer.parseInt(((_section==null)? "1":_section) );
				int pageNum = Integer.parseInt(((_pageNum==null)? "1":_pageNum));
				// 글 목록에서 명시적으로 페이지 번호를 눌러서 요청한 경우 section 값과 pageNum값을 가져온다.
				...
				if (imageFileName != null && imageFileName.length() != 0) {
					...
					oldFile.delete();
					// 전송된 originalImageFileName을 이용해 기존의 파일을 삭제한다.
				}
				...
				// articleNO 값에 대한 글을 삭제한 후 삭제된 부모 글과 자식 글의 articleNO 목록을 가져온다.
				...
				// 답글창 요청 시 미리 부모 글 번호를 parentNO속성으로 세션에 저장한다.
